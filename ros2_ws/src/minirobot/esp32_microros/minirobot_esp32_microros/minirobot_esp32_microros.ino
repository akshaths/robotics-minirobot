...
int map_to_percentage(int raw_value) {
  ...
  int min_voltage = 2180;    // Minimum voltage reading (corresponding to 0% battery)
  int max_voltage = 3060; // Maximum voltage reading (corresponding to 100% battery)
...
void setup() {
  ...
  set_microros_wifi_transports("AI", "*****", "192.168.7.63", 8888);
  ...
  // create node
  RCCHECK(rclc_node_init_default(&node, "minirobot_esp32", "", &support));
